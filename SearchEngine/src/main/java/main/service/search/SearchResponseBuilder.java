...
public class SearchResponseBuilder implements Runnable {
    ...
    private void defineLemmaFrequencies() {
        if (request.getSiteUrls().size() == 1) {
            site = RepositoryMapper.siteRepo.findByUrlAndType(request.getSiteUrls().get(0), Site.INDEXED)
                    .orElse(null);
            if (site == null) {
                return;
            }
    ...
    private void defineRelevancesForFirstLemma(String lemma) {
        List<Index> indices = site != null ?
                RepositoryMapper.indexRepo.findAllByTextLemmaAndSite(lemma, site) :
                RepositoryMapper.indexRepo.findAllByTextLemma(lemma);
