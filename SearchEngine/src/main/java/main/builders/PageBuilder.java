...
public class PageBuilder implements Runnable {
    ...
    public PageBuilder(Site site, String pagePath) {
        this.site = site;
        oldPages = RepositoryMapper.pageRepo.findAllBySiteAndPathAndCode(site, pagePath, Node.OK);
        ...
        page = RepositoryMapper.pageRepo.findById(id).orElse(null);
        if (page == null) {
            doc = null;
            return;
        }
    ...
    public void run() {
        ...
        List<Lemma> lemmaList = RepositoryMapper.lemmaRepo.findAllBySite(site);
        Map<String, Lemma> lemmas = new HashMap<>();
        for (Lemma lemma : lemmaList) {
            lemmas.put(lemma.getLemma(), lemma);
        }

        List<Index> indexList = RepositoryMapper.indexRepo.findAllBySite(site);
        ...
        RepositoryMapper.lemmaRepo.deleteAllInBatch(lemmasToDelete);
        ...
        synchronized (Page.class) {
            RepositoryMapper.pageRepo.saveAndFlush(page);
        }
        RepositoryMapper.lemmaRepo.deleteAllInBatch(lemmasToDelete);
        RepositoryMapper.lemmaRepo.saveAllAndFlush(lemmaList);
        RepositoryMapper.indexRepo.saveAllAndFlush(pageIndices);
        synchronized (Page.class) {
            if (oldPages != null) {
                for (Page p : oldPages) {
                    RepositoryMapper.pageRepo.deleteById(p.getId());
                }
            }
        }
    ...
    public static String indexPage(String stringUrl) {
        ...
        Site site = RepositoryMapper.siteRepo.findByUrlAndType(home, Site.INDEXED).orElse(null);
