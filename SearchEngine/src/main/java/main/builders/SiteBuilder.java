...
public class SiteBuilder implements Runnable {
    ...
    final static int forSitesThreadNumber =
            Props.getInst().getThreadNumberForSites();
    ...
    public SiteBuilder(String siteUrl) {
        ...
        Optional<Site> indexingSite =
                RepositoryMapper.siteRepo.findByUrlAndType(siteUrl, Site.INDEXING);
        if (!indexingSite.isEmpty()) {
            indexingSite.get().setType(Site.REMOVING);
            RepositoryMapper.siteRepo.saveAndFlush(indexingSite.get());
        }
        ...
        RepositoryMapper.siteRepo.saveAndFlush(site);
    }
    ...
    public void run() {
        ...
        if (isStopping()) {
            Site indexingSite = RepositoryMapper.siteRepo.findByNameAndType(site.getName(), Site.INDEXING)
                    .orElse(null);
            if (indexingSite != null) {
                indexingSite.setType(Site.REMOVING);
                RepositoryMapper.siteRepo.saveAndFlush(indexingSite);
            }
    ...
    private void setCurrentSiteAsWorking() {
        Site prevSite = RepositoryMapper.siteRepo.findByNameAndType(site.getName(), Site.INDEXED)
                .orElse(null);
        if (prevSite == null) {
            prevSite = RepositoryMapper.siteRepo.findByNameAndType(site.getName(), Site.FAILED)
                    .orElse(null);
        }
        if (prevSite != null) {
            prevSite.setType(Site.REMOVING);
            RepositoryMapper.siteRepo.saveAndFlush(prevSite);
        }
        ...
        synchronized(Site.REMOVING) {
            RepositoryMapper.siteRepo.deleteByType(Site.REMOVING);
        }
    }
